...
## 2025-01-XX — Rewrite fetch_overview to Use Playwright Locators

**Task:**
Rewrite `fetch_overview` method in `src/tv_data_fetcher.py` to use Playwright's locator API instead of grabbing entire `#tv-content` innerText. The goal was to target the "Fundamentals and stats" section specifically to reduce noise.

**Implementation:**
- Changed method to open a page directly (no longer uses `_fetch_page_text()`)
- Uses `page.locator('h1:has-text("Fundamentals and stats")').locator('..')` to find the H1 and its parent container
- Waits for element visibility with 10s timeout
- Extracts `inner_text()` from parent container
- Includes graceful fallback: if locator fails (timeout or element not found), falls back to old `_fetch_page_text()` approach
- Properly closes page in finally block (with guard to prevent double-close during fallback)
- Maintains compatibility with `_with_retry` wrapper
- Keeps "STOCK OVERVIEW\n\n" prefix

**Files Modified:**
- src/tv_data_fetcher.py — Rewrote `fetch_overview` method (lines 96-132)

**Verification Method:**
- Python syntax check passed with `python -m py_compile`
- Code follows existing error handling patterns
- Maintains same return format for compatibility with rest of system

**Learnings:**
- Playwright Python API: `page.locator()` with CSS selectors, `.locator('..')` for parent traversal
- `locator.wait_for(state="visible", timeout=...)` for explicit waits
- `locator.inner_text()` to extract text content
- Graceful degradation pattern: try new approach, fall back to proven method on failure
- Managing page lifecycle: need to track page state to avoid double-close in fallback scenario
